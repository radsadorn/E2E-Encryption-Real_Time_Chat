...
export class AuthorizationService {
  ...
  public async createNewUser(newUser: NewUser): Promise<string> {
    try {
      console.log(
        this.LOG_NAME + 'Create New User : ' + JSON.stringify(newUser)
      );

      // find by username
      // username collision
      const result = this.users[newUser.username];
      if (result) throw { message: 'Username is already exist!!!' };
  ...
  public async login(userLogin: NewUser): Promise<string> {
    try {
      console.log(this.LOG_NAME + 'User login : ' + JSON.stringify(userLogin));

      // find by username
      const result = this.users[userLogin.username];
      if (!result) return await this.createNewUser(userLogin);
